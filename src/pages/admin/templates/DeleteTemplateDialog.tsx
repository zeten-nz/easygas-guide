...
export function DeleteTemplateDialog({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(`"${template.name}" o'chirildi`);
  ...
  return (
    <Modal open onClose={mutation.isPending ? () => {} : onClose} title="Shablonni o'chirish" className="sm:max-w-md">
      <div className="space-y-4">
        ...
        <div className="flex items-start gap-2.5 rounded-2xl border border-brand-500/25 bg-brand-50 p-3.5">
          ...
          <p className="text-[13px] leading-relaxed text-[var(--text-2)]">
            <b className="text-[var(--text-1)]">"{template.name}"</b> butunlay o'chiriladi. Bu amalni ortga qaytarib
            bo'lmaydi.
          </p>
        </div>

        <p className="text-sm leading-relaxed text-[var(--text-2)]">
          Faqat hech qachon nashr qilinmagan qoralama shablonni o'chirish mumkin. Nashr qilingan yoki ishlarga
          biriktirilgan shablon <b className="text-[var(--text-1)]">o'chirilmaydi</b> — uning tarixi saqlanadi; buning
          o'rniga versiyani arxivlang.
        </p>
