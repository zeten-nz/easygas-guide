...
test('names the template and explains permanent-delete vs archive', () => {
  ...
  renderWithProviders(<DeleteTemplateDialog template={template} onClose={() => {}} />);
...
test('a server conflict shows the reason and does NOT pretend success', async () => {
  ...
  renderWithProviders(<DeleteTemplateDialog template={template} onClose={onClose} onDeleted={onDeleted} />);
...
test('a successful delete fires onDeleted', async () => {
  ...
  const onDeleted = vi.fn();
  renderWithProviders(<DeleteTemplateDialog template={template} onClose={() => {}} onDeleted={onDeleted} />);
  await userEvent.click(screen.getByRole('button', { name: "O'chirish" }));
